...
{-
What if we fuse deeper? What if we used a syntactic monad for the embedded effects,
so we could fuse those too? That way some of thse fmaps and binds can be optimized away...
-}
...
takeWhileC :: Functor m => (Code Q a -> Code Q Bool) -> Stream a m r -> Stream a m ()
...
scanC :: Monad m => (Code Q x -> Code Q a -> Code Q x) -> Code Q x -> (Code Q x -> Code Q b) -> Stream a m r -> Stream b m r
scanC step begin done (S cs0 next) = S [|| ($$cs0,$$begin) ||] (\csx k -> [||
        let !(s,x) = $$csx in
        $$(next [||s||] (\case
            Effect cms' -> k (Effect [|| (,x) <$> $$cms' ||])
            Tau cms' -> k (Tau [|| ($$cms',x) ||])
            Yield ca cs' -> [||
                let !x' = $$(step [|| x ||] ca) in
                $$(k (Yield (done [|| x' ||]) [|| ($$cs',x') ||]))
...
effects :: Monad m => Stream a m r -> Code Q (m ())
effects s = [|| (Control.Monad.void $$(foldC (\_ _ -> [|| () ||]) [|| () ||] (const [|| () ||]) s))||]